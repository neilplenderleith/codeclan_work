...
library(tidyverse)
library(janitor)
movies <- read.csv("errors/../data/movies.csv")
```
...
# Summarises the NA values for each of the columns
```
...
movies_runtime <- movies %>% 
  mutate(runtime = na_if(runtime, 0)) # convert 0s to NAs
...
movies_imputed <- movies_runtime %>% 
  mutate(runtime = coalesce(runtime, 
                            median(runtime, 
                                   na.rm = TRUE))) # remember the na.rm = TRUE ARG!
...
# selects all character columns, then summarises with the sum of their NA values
...
# selects all numeric columns, then summarises with the sum of their NA values
